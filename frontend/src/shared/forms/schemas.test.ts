...
describe('form schemas', () => {
  ...
  it('validates product and password forms', () => {
    expect(
      productCreateSchema.safeParse({ title: 'Cam', brand: '', model: '', currentValue: '' })
        .success
    ).toBe(true)
    expect(
      productCreateSchema.safeParse({ title: 'Cam', brand: '', model: '', currentValue: 'abc' })
        .success
    ).toBe(false)
    expect(
      productCreateSchema.safeParse({ title: 'Cam', brand: '', model: '', currentValue: '-10' })
        .success
    ).toBe(false)
    expect(
      changePasswordSchema.safeParse({ currentPassword: 'old', newPassword: '12345678' }).success
    ).toBe(true)
