...
export default function ContentKanbanPageView() {
  ...
  const loadAll = useCallback(async () => {
    const [itemsPage, tasksPage, profileData, templateData] = await Promise.all([
      api<ApiPage<ContentItem>>("/content/items?limit=100&offset=0&sort_by=updated_at&sort_order=desc"),
      api<ApiPage<ContentTask>>("/content/tasks?limit=400&offset=0&sort_by=updated_at&sort_order=desc"),
  ...
  const calendarItems = useMemo(
    () => [...items].sort((a, b) => `${a.publish_date || a.planned_date || "9999"}`.localeCompare(`${b.publish_date || b.planned_date || "9999"}`)),
    [items],
  );
    ...
    <div className="container board-layout">
                    ...
                    {column.items.map((item) => (
                      ...
                      >
                        ...
                        <div className="small muted">
                          {item.platform.toUpperCase()} • {item.type.toUpperCase()} • {item.readiness_score}%
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div className="card">
                  ...
                  <div className="stack">
                    <select
                      ...
                      onChange={(event) => updateItem(selected.id, { status: event.target.value }).catch(alert)}
                    >
                      ...
                      return (
                        <div key={key}>
                          ...
                          <input
                            ...
                            onChange={(event) => updateDynamicField(key, event.target.value).catch(alert)}
                          />
                        </div>
                      );
                    })}
                        ...
                        <div className="control-row section-gap">
                          {["todo", "doing", "done"].map((status) => (
                            <button
                              ...
                              onClick={() => updateTask(task.id, { status: status as ContentTask["status"] }).catch(alert)}
                            >
      ...
      {activeTab === "calendar" && (
        <div className="card stack">
          ...
          {calendarItems.length === 0 && <div className="muted">{t("contentHub.noPlannedContent")}</div>}
        </div>
      )}
        ...
        <div className="card stack">
          {!selectedId || !planning ? (
            ...
            <>
              ...
              <div>
                {t("contentHub.publishReady")}: {planning.publish_ready ? t("common.yes") : t("common.no")}
              </div>
              {planning.blockers.length > 0 && (
                <div className="stack">
                  {planning.blockers.map((blocker) => (
                    <div key={blocker} className="pill">{blocker}</div>
                  ))}
                </div>
              )}
              {templates.length > 0 && (
                <div className="control-row">
                  {templates.slice(0, 4).map((template) => (
                    <button key={template.id} className="btn" onClick={() => applyTemplate(template.id).catch(alert)}>
            ...
            {templates.map((template) => (
              <div key={template.id} className="card tight">
                ...
                <div className="muted small">
                  {template.applies_to_platform || t("contentHub.all")} • {template.applies_to_type || t("contentHub.all")}
                </div>
                <div className="small">{t("contentHub.templateItemsCount", { count: template.items.length })}</div>
              </div>
            ))}
