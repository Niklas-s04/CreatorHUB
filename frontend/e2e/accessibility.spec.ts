...
import { acceptNecessaryCookies } from './helpers'
  ...
  test('login page meets baseline WCAG checks and exposes consent actions', async ({ page }) => {
    ...
    await expect(
      page.getByRole('region', {
        name: /necessary cookies are required|notwendige cookies sind erforderlich/i,
      })
    ).toBeVisible()
    await expect(
      page.getByRole('button', { name: /necessary only|nur notwendige cookies/i })
    ).toBeVisible()
    await expect(
      page.getByRole('button', { name: /allow analytics|analytics erlauben/i })
    ).toBeVisible()
