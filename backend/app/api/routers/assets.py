...
def _upload_purpose_allowed(owner_type: AssetOwnerType, kind: AssetKind) -> bool:
    if kind == AssetKind.pdf and owner_type not in {
        AssetOwnerType.product,
        AssetOwnerType.deal,
        AssetOwnerType.content,
    }:
        return False
    if kind == AssetKind.image:
        return True
...
def _delivery_headers(asset: Asset, path: str) -> tuple[str | None, str, dict[str, str]]:
    ...
    cache_control = (
        "private, max-age=300" if asset.review_state == AssetReviewState.approved else "no-store"
    )
    headers = {
...
def update_asset(
    ...
    if (
        ...
        and data.get("review_state")
        and original_review_state != asset.review_state
    ):
        record_audit_log(
            db,
            actor=current_user,
